...
雲端 (GitHub Actions, 免費)  每 5 分鐘：
  Finnhub 新聞+報價+基本面 · Stooq 日線(ATR/均線/區間) · StockTwits 散戶情緒
   → Gemini 判 {多空 · 信心 · 已證實/傳聞 · 是否改變長期論點}
...
資料來源**全部免費**：**Finnhub**（新聞/報價/基本面/分析師）、**Stooq**（日線歷史，免金鑰）、**StockTwits**（散戶情緒，盡力而為）、**Google Gemini**（判讀，免費 `gemini-2.5-flash`）。
...
| **Google Gemini** | 新聞判讀（免費）| https://aistudio.google.com/apikey |
...
2. repo **Settings → Secrets and variables → Actions** 設 secret：`FINNHUB_API_KEY`、`GEMINI_API_KEY`（必填）、`TELEGRAM_BOT_TOKEN`、`TELEGRAM_CHAT_ID`、`POSITIONS_JSON`（選用）。
...
| **你的真實持倉 + 帳戶大小/風險%** | `config/positions.json`（公開 repo 請改用 `POSITIONS_JSON` secret） |
| 新聞判讀邏輯 | `lib/analyze.ts`（Gemini 提示詞） |
...
> 價格一律由公式算，**AI 不參與算價**（只判新聞）。

## 6. 成本 / 額度

- **Finnhub**：免費 60 calls/分鐘。
- **Google Gemini**：免費（AI Studio，`gemini-2.5-flash`）；有免費層 RPM/RPD 限制，量小通常夠。
- **Stooq / StockTwits / Telegram / Vercel**：免費。
- **GitHub Actions**：Public repo 無上限；Private 免費 2000 分鐘/月。
...
config/
  ...
  positions.json     你的真實持倉 + 帳戶設定（公開 repo 改用 POSITIONS_JSON secret）
lib/
  finnhub.ts         報價 / 新聞 / 基本面
  marketdata.ts      Stooq 日線 → ATR/均線/區間/量能
  stocktwits.ts      散戶情緒（選用）
  analyze.ts         Gemini 判讀（多空/信心/傳聞/長期影響）
...
- **連富途/moomoo（本機）**：用 FutuOpenD + Python sidecar 接**即時報價 + 真實持倉自動同步**；下單先用**模擬盤 + 一鍵確認**，不做全自動。
- 更多新聞源、價格異動警報、情緒時間序列回看。
